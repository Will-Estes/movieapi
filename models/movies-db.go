...
func (m *DBModel) Get(id int) (*Movie, error) {
	...
	genres, err := getMovieGenres(ctx, id, m)
	if err != nil {
		return nil, err
	}
...
func (m *DBModel) All(genre ...int) ([]*Movie, error) {
	...
	movieRows, err := m.DB.QueryContext(ctx, query)
	if err != nil {
		return nil, err
	}
	...
	for movieRows.Next() {
		...
		genres, err := getMovieGenres(ctx, movie.ID, m)
		if err != nil {
			return nil, err
		}
		movie.MovieGenre = genres
...
func getMovieGenres(ctx context.Context, id int, m *DBModel) (map[int]string, error) {
...
func (m *DBModel) UpdateMovie(movie Movie) error {
	...
	stmt := `update movies
			set title = $1, description = $2, year = $3, release_date = $4, rating = $5, runtime = $6, 
